...
function ApplicationCreatePage() {
  ...
  const formik = useFormik<ApplicationInterface>({
    initialValues: {
      status: '',
      applied_date: new Date(new Date().toDateString()),
      resume: '',
      cover_letter: '',
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <FormControl id="applied_date" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Applied Date
            </FormLabel>
            <DatePicker
              selected={formik.values?.applied_date ? new Date(formik.values?.applied_date) : null}
              onChange={(value: Date) => formik.setFieldValue('applied_date', value)}
            />
          </FormControl>
